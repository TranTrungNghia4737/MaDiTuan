...
namespace ProjectAI_by_ConsoleApp
{
    class Program
    {
        ...
        static void solveKT()
        {
            ...
            {
                Console.WriteLine("Không tìm thấy đường đi khả thi cho ô {0}{1}" + startX, startY);
            }
            else
            {
                printBoard(boardGrid);
                Console.WriteLine("Số đường đi đã thử {0}", attemptedMoves);
            }
            ...
            bool solveKTUtil(int x, int y, int movecount)
            {
                ...
                if (attemptedMoves % 1000000 == 0)
                    Console.WriteLine("Đã thử được {0} đường đi", attemptedMoves);
                int k;
                int next_X;
                int next_Y;
